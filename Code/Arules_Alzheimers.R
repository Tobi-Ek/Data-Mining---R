...
# ------- The BayesLCA data set - Alzheimer Syndrome ------- 
...
# convert Alzheimer data set into a Formal class transaction
...
# A key result which is used in the apriori algorithm is:
# P(S1,S2) <= min{(S1),P(S2)}
...
modelfit <- apriori(Alzheimer.trans,parameter=list(confidence=0.5,support=0.01,minlen=2,maxlen=5))
modelfit <- sort(modelfit,by="support")
inspect(modelfit)

# Scatter Plot
plot(modelfit, measure=c("support", "lift"), shading="confidence")
quality(modelfit)$
# Matrix Plot
subfit <- modelfit[quality(modelfit)$confidence > 0.8]
plot(subfit, method="matrix", measure="lift", control=list(reorder = measure))

# Grouped Plot
plot(subfit, method="group")
...
# We have seen that the range of values that lift takes can depend on:
# the minimum support (s) and confidence (c) thresholds used.
